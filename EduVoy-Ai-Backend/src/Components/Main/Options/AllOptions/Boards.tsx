...
const Boards: React.FC = () => {
    ...
    const goToUpadteOption = async (idx: number) => {
        ...
        try {
            ...
            const response = await fetch(`${BACKEND_URL}/admin/options/board/${idx}`, {
                method: "GET",
                headers: {
                    'token': `${token}`
                },
            ...
            const universityToUpdate = data.board;
    ...
    const updateOption = () => {
        ...
        if (optionTitle) {
            fetch(`${BACKEND_URL}/admin/options/update/board`, {
                method: "PUT",
                headers: {
                    'token': `${token}`,
    ...
    const deleteOption = (idx: number) => {
        ...
        fetch(`${BACKEND_URL}/admin/options/delete/board`, {
            method: "DELETE",
            headers: {
                'token': `${token}`,
                "Content-Type": "application/json"
    ...
    const addOption = () => {
        ...
        if (optionTitle) {
            ...
            fetch(`${BACKEND_URL}/admin/options/add/board`, {
                method: "POST",
                headers: {
                    'token': `${token}`,
                    "Content-Type": "application/json",
            ...
            {!isFetching && <div className="max-w-7xl mx-auto p-6 -mt-6 -ml-6">
                <div className='flex justify-center'>
                    <div ref={inputSectionRef} className="flex justify-around bg-gradient-to-r from-blue-500 to-purple-600 rounded-xl p-5 mb-8 text-white w-[1000px]">
                        ...
                        >
                            {isOptionToUpdated ? "Update Board" : "Add Board"}
                        </motion.button>
                    </div>
                </div>

...
import { inActionUniIdAtom, stateOfChangesAtom, isUgActionedAtom, isFetchingAtom } from './Atoms';
import { UgUnis } from './../../../utils/ugunis';
...
const UgUniversities: React.FC = () => {
    ...
    const isActioned = useRecoilValue(isUgActionedAtom);
    ...
    const [ugUniOptions, setUgUniOptions] = useState<number[]>([]);
    ...
    const [ugUnis, setUgUnis] = useState<UgUnis[]>([]);
    ...
    const fetchUgUnis = async () => {
        try {
            ...
            const response = await fetch(`${BACKEND_URL}/admin/universities/uguniversities`, {
                method: "GET",
                headers: {
                    'token': `${token}`
                },
            });
            const data: { ugUniversities: UgUnis[] } = await response.json();
            setUgUnis(data.ugUniversities);

            let options = [];
            options.push(...data.ugUniversities.map(obj => (obj.universityId)));
            setUgUniOptions(options);
    ...
    function listToBeAddedUnis() {
        ...
        uniOptions.forEach(uni => {
            if (!ugUniOptions.includes(uni as number)) {
                toBeAddedUnis.push({
                    value: uni,
    ...
    function deleteUniReq(idx: number) {
        ...
        fetch(`${BACKEND_URL}/admin/universities/uguniversities/delete`, {
            method: "DELETE",
            headers: {
                'token': `${token}`,

...
import { ScholarshipsType } from './../../../../../../Utils/scholarship';
import { useEffect, useState, useRef } from 'react';
import { useRecoilValue, useSetRecoilState } from 'recoil';
import { GraduationCap, ExternalLink, ChevronDown, CheckCircle, Cross } from 'lucide-react';
import { userDetailsAtom, scholarshipListAtom } from './../../../../../../Atoms/atoms';
...
const Scholarships = () => {
    ...
    //For Short List
    const setShortList = useSetRecoilState(scholarshipListAtom);
    const shortList = useRecoilValue(scholarshipListAtom);
    ...
    const updateShortList = (scholarships: number[]) => {
        ...
        fetch(`${BACKEND_URL}/users/updateField/scholarships/${userDetails.id}`, {
            ...
            body: JSON.stringify({ updatingField: { scholarships } }),
        })
            .then(async (res) => {
                if (!res.ok) {
    ...
    const fetchResources = async () => {
        try {
            ...
            const response = await fetch(`${BACKEND_URL}/users/finResources/scholarship`, {
                method: "GET",
                headers: {
                    'token': `${token}`
                },
            });
            const data: { resources: ScholarshipsType[] } = await response.json();
                ...
                <div className='mb-10 flex justify-end items-center gap-4'>
                    ...
                    <div className='selectBorder'>
                        <Select
                            ...
                            closeOnClickInput
                            values={[{ value: 'false', label: 'All Scholarships' }]}
                            options={[{ value: 'false', label: 'All Scholarships' }, { value: 'true', label: 'Shortlisted Scholarships' }]}
                            onChange={(value): void => { setFilter(value[0].value) }}
                        />
                    </div>
                </div>
                {isFetched && resources.length === 0 && <div className="font-bold text-xl text-center w-full">Sorry! There are no shortlisted scholarships.</div>}
                    ...
                    {resources.map((resource, index) =>
                        ...
                        >
                                ...
                                <div className="flex-1 mt-3">
                                    ...
                                    <div className='grid grid-cols-2'>
                                        <p className="font-light text-xl mb-2">
                                            <b className="font-bold text-lg mb-3">Provider:</b> {resource.provider}
                                        </p>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <a href={resource.link} target='blank' className='btn btn-primary w-full text-center flex justify-center items-center gap-1'>Apply Now<ExternalLink /></a>
                                <button

...
const Intakes: React.FC = () => {
    ...
    const goToUpadteOption = async (idx: number) => {
        ...
        try {
            ...
            const response = await fetch(`${BACKEND_URL}/admin/options/intake/${idx}`, {
                method: "GET",
                headers: {
                    'token': `${token}`
                },
            ...
            const universityToUpdate = data.intake;
    ...
    const updateOption = () => {
        ...
        if (optionTitle) {
            fetch(`${BACKEND_URL}/admin/options/update/intake`, {
                method: "PUT",
                headers: {
                    'token': `${token}`,
    ...
    const deleteOption = (idx: number) => {
        ...
        fetch(`${BACKEND_URL}/admin/options/delete/intake`, {
            method: "DELETE",
            headers: {
                'token': `${token}`,
                "Content-Type": "application/json"
    ...
    const addOption = () => {
        ...
        if (optionTitle) {
            ...
            fetch(`${BACKEND_URL}/admin/options/add/intake`, {
                method: "POST",
                headers: {
                    'token': `${token}`,
                    "Content-Type": "application/json",
            ...
            {!isFetching && <div className="max-w-7xl mx-auto p-6 -mt-6 -ml-6">
                <div className='flex justify-center'>
                    <div ref={inputSectionRef} className="flex justify-around bg-gradient-to-r from-blue-500 to-purple-600 rounded-xl p-5 mb-8 text-white w-[1000px]">
                        ...
                        >
                            {isOptionToUpdated ? "Update Intake" : "Add Intake"}
                        </motion.button>
                    </div>
                </div>

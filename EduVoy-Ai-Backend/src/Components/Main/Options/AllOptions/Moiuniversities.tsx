...
const MoiUniversities: React.FC = () => {
    ...
    const goToUpadteOption = async (idx: number) => {
        ...
        try {
            ...
            const response = await fetch(`${BACKEND_URL}/admin/options/moiunis/${idx}`, {
                method: "GET",
                headers: {
                    'token': `${token}`
                },
    ...
    const updateOption = () => {
        ...
        if (optionTitle) {
            fetch(`${BACKEND_URL}/admin/options/update/moiunis`, {
                method: "PUT",
                headers: {
                    'token': `${token}`,
    ...
    const deleteOption = (idx: number) => {
        ...
        fetch(`${BACKEND_URL}/admin/options/delete/moiunis`, {
            method: "DELETE",
            headers: {
                'token': `${token}`,
                "Content-Type": "application/json"
            ...
            {!isFetching && <div className="max-w-7xl mx-auto p-6 -mt-6 -ml-6">
                <div className='flex justify-center'>
                    <div ref={inputSectionRef} className="flex justify-around bg-gradient-to-r from-blue-500 to-purple-600 rounded-xl p-5 mb-8 text-white w-[1000px]">
                        ...
                        >
                            {isOptionToUpdated ? "Update University" : "Add University"}
                        </motion.button>
                    </div>
                </div>
...
export default MoiUniversities
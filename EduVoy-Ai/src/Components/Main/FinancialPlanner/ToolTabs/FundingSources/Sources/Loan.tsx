...
import { LoansType } from './../../../../../../Utils/loanType';
import { useEffect, useState, useRef } from 'react';
import { useRecoilValue, useSetRecoilState } from 'recoil';
import { CreditCard, ExternalLink, ChevronDown, CheckCircle, Cross } from 'lucide-react';
import { userDetailsAtom, loanListAtom } from './../../../../../../Atoms/atoms';
...
const Loans = () => {
    ...
    //For Short List
    const setShortList = useSetRecoilState(loanListAtom);
    const shortList = useRecoilValue(loanListAtom);
    ...
    const updateShortList = (loans: number[]) => {
        ...
        fetch(`${BACKEND_URL}/users/updateField/loans/${userDetails.id}`, {
            ...
            body: JSON.stringify({ updatingField: { loans } }),
        })
            .then(async (res) => {
                if (!res.ok) {
    ...
    const fetchResources = async () => {
        try {
            ...
            const response = await fetch(`${BACKEND_URL}/users/finResources/loan`, {
                method: "GET",
                headers: {
                    'token': `${token}`
                },
            });
            const data: { resources: LoansType[] } = await response.json();
                ...
                <div className='mb-10 flex justify-end items-center gap-4'>
                    ...
                    <div className='selectBorder'>
                        <Select
                            ...
                            closeOnClickInput
                            values={[{ value: 'false', label: 'All Loans' }]}
                            options={[{ value: 'false', label: 'All Loans' }, { value: 'true', label: 'Shortlisted Loans' }]}
                            onChange={(value): void => { setFilter(value[0].value) }}
                        />
                    </div>
                </div>
                {isFetched && resources.length === 0 && <div className="font-bold text-xl text-center w-full">Sorry! There are no shortlisted loans.</div>}
                                ...
                                <div className="flex-1 mt-3">
                                    ...
                                    <div className='grid grid-cols-2'>
                                        <p className="font-light text-xl mb-2">
                                            <b className="font-bold text-lg mb-3">Provider:</b> {resource.bankName}
                                        </p>
                                    ...
                                    <div className='grid grid-cols-2'>
                                        ...
                                        <p className="font-light text-xl mb-2">
                                            <b className="font-bold text-lg mb-3">Processing Fee:</b> {resource.processingFee}
                                        </p>
                                    </div>
                                </div>
                            </div>
